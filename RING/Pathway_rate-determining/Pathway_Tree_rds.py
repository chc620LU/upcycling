...
df_RING_Table = pd.read_csv("./RING_Summary.csv",index_col=0,squeeze=True)
dic = {}
for i in range(len(df_RING_Table)):
    dic[df_RING_Table['Reactions'][i+1]] = [df_RING_Table['Rules'][i+1], df_RING_Table['delta G'][i+1]]

rds_define = ['Dehydrogenation']
# rds_define = ['BetaScission','EndMethaneBetaScissionWithHB2']
# rds_define = ['FirstCyclization','SecondCyclizationtypeA','SecondCyclizationtypeB']
# rds_define = ['PCP']

print('rds_define:', rds_define)
for i in range(78):
    filename = "../Pathway_Analysis_Result/Pathways_AnaResult_AA_Jun30_%s.txt" % i
    print(filename[-33:-4])
